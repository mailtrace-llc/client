...
const SRC      = process.argv[2] ? path.resolve(process.argv[2]) : path.resolve('figma-node.json')
...
function toHex({ r, g, b }, opacity = 1) {
  ...
  return opacity === 1 ? hex : `rgba(${Math.round(r*255)}, ${Math.round(g*255)}, ${Math.round(b*255)}, ${opacity})`
}
...
function strokeInfo(node) {
  ...
  return { color: toHex(s.color, s.opacity ?? 1), w: num(node.strokeWeight) ?? 1 }
}
...
function byCanvasPos(a, b) {
  ...
  const ay = Math.round(A.y ?? 0), by = Math.round(B.y ?? 0)
  const ax = Math.round(A.x ?? 0), bx = Math.round(B.x ?? 0)
  return ay === by ? ax - bx : ay - by
}

const ALLOW = new Set(['FRAME', 'TEXT', 'RECTANGLE', 'ELLIPSE', 'VECTOR', 'LINE', 'GROUP', 'INSTANCE'])

// palette collector
const palette = new Map()
const addColor = (hex) => { if (!hex) return; palette.set(hex, (palette.get(hex) || 0) + 1) }
...
function simplify(node, index = 0, parentPath = []) {
  ...
  if (node.layoutMode) {
    base.layout = {
      ...
      padding: {
        t: num(node.paddingTop), r: num(node.paddingRight),
        b: num(node.paddingBottom), l: num(node.paddingLeft)
      },
  ...
  if (stroke) { base.stroke = stroke; addColor(stroke.color) }
  if (Array.isArray(node.effects) && node.effects.length) base.shadow = true
  if (node.clipsContent) base.clip = true
  if (typeof node.opacity === 'number' && node.opacity !== 1) base.opacity = +node.opacity.toFixed(3)
  ...
  if (node.type === 'TEXT') {
    ...
    if (color) { base.color = color; addColor(color) }
...
function outlineMD(n, depth = 0) {
  ...
  const layout =
    n.layout ? ` • ${n.layout.mode}` +
      (n.layout.gap != null ? ` gap:${n.layout.gap}` : '') +
      (n.layout.padding ? ` pad:${['t','r','b','l'].map(k=>n.layout.padding[k]??0).join('/')}` : '')
      : ''
  const bits = [
    `- ${n.type}${n.name ? ` • ${n.name}` : ''}${n.kind ? ` • ${n.kind}` : ''}${box}${layout}`,
  ]
  if (n.text) bits.push(pad + `  " ${n.text.replace(/\s+/g,' ').slice(0,80)}${n.text.length>80?'…':''} "`)
  if (Array.isArray(n.children)) for (const c of n.children) bits.push(outlineMD(c, depth + 1))
  return bits.join('\n')
}
...
const mdHead =
  ...
  (summary._palette?.length
    ? `**Palette (top ${Math.min(12, summary._palette.length)}):** ` +
      summary._palette.slice(0,12).map(p=>`\`${p.hex}\`×${p.count}`).join('  ') + `\n\n`
    : '')
